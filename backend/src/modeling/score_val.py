"""Score the persisted LR baseline on season-2 VALIDATION, once, and save
the predictions to disk -- mirrors `score_test.py` exactly, but for the
val split, so downstream calibration work (fitting an isotonic
recalibrator, etc.) has a persisted per-row prediction artifact to fit
against without ever retraining or re-touching test.

**Not a new test-set decision, not a retrain.** `train_lr.py --full`
already scored every `C` in the grid against val as part of model
selection (see `backend/data/metadata/ctr_lr_runs.jsonl` and the
`val_selection_metrics` block recorded for the selected model in
`ctr_lr_test_evaluation_season2.json`) -- but it never persisted the
...
loads the already-fitted artifact (`ctr_lr_baseline_season2.joblib`,
selected `C` baked in) and reproduces the exact forward pass over val that
`train_lr.py` already made when it picked that `C`, so the resulting
per-row probabilities can be persisted and reused (e.g. for fitting an
isotonic calibrator on val only, never on test).

Because nothing here is stochastic (no fitting happens), re-running it is
idempotent, and its pooled AUC/log loss/ECE are asserted, at import-safe
runtime, to match the existing `val_selection_metrics` recorded for the
selected `C` in `ctr_lr_test_evaluation_season2.json` -- see
`_assert_matches_recorded_val_selection()` below. If that assertion ever
fails, report it loudly rather than silently accepting a new number (same
policy as `score_test.py`).

Usage (from `backend/`, using the project venv):

    backend\\.venv\\Scripts\\python.exe -m src.modeling.score_val
"""
...
_OUTPUT_PARQUET_PATH = _PREDICTIONS_ROOT / "ctr_lr_baseline_season2_val.parquet"
_OUTPUT_SIDECAR_PATH = _PREDICTIONS_ROOT / "ctr_lr_baseline_season2_val.json"

# Same carry-columns convention as score_test.py.
...
def _assert_matches_recorded_val_selection(result, *, atol: float = 1e-9) -> None:
    ...
    `val_selection_metrics` block already recorded for the SELECTED model
    in `ctr_lr_test_evaluation_season2.json` (written once by
    `train_lr.py --full --touch-test`, from the same fit/`C` that produced
    the persisted `ctr_lr_baseline_season2.joblib` artifact this script
    loads). A mismatch means this script's forward pass over val diverged
    from the one that picked the selected `C` -- fail loudly.
    """
    if not _RECORDED_TEST_EVAL_PATH.exists():
        raise FileNotFoundError(
            ...
            "be recorded there; run `train_lr.py --full --touch-test` "
    ...
    if mismatches:
        raise AssertionError(
            "score_val.py: this run's forward pass does NOT reproduce the "
            f"recorded val_selection_metrics at {_RECORDED_TEST_EVAL_PATH}. "
...
def _assert_persisted_column_reproduces_harness(
    ...
    *,
    ...
    """Same guard as `score_test.py`'s equivalent function: recompute
    ...
    if problems:
        raise AssertionError(
            "score_val.py: locally-recomputed metrics on the array about "
...
def main(argv=None) -> int:
    ...
    print(f"[score_val] loading model artifact -> {_MODEL_PATH}")
    if not _MODEL_PATH.exists():
        print(
            f"[score_val] REFUSING to proceed: {_MODEL_PATH} does not exist. "
    ...
    if saved_boundaries != live_boundaries_dict:
        print(
            "[score_val] REFUSING to proceed: artifact's saved "
    ...
    print(
        f"[score_val] artifact: best_C={best_C} sampling_rate={sampling_rate} "
        f"split_boundaries={saved_boundaries}"
    )

    print("[score_val] loading season-2 val split (true rate, never downsampled) ...")
    X_val_raw, y_val = split.load_split(2, "val")
    print(f"[score_val] val: {X_val_raw.shape}, positives={int(y_val.sum())}")

    print("[score_val] encoding val features with the SAVED scaler (fit_scaler=False) ...")
    X_val_enc, _ = features_lr.encode_features(X_val_raw, scaler=scaler, fit_scaler=False)

    print("[score_val] scoring (single forward pass, no retraining) ...")
    p_raw = model.predict_proba(X_val_enc)[:, 1]
    ...
    print(
        f"[score_val] pooled val metrics: auc={result.auc:.10f} "
        f"log_loss={result.log_loss:.10f} ece={result.ece:.10f}"
    )

    print("[score_val] cross-checking against the recorded val_selection_metrics ...")
    _assert_matches_recorded_val_selection(result)
    print(f"[score_val] OK: matches val_selection_metrics in {_RECORDED_TEST_EVAL_PATH} exactly (within 1e-9).")
    ...
    print("[score_val] OK: persisted recalibrated-probability column reproduces the harness's own metrics exactly.")

    print("[score_val] assembling predictions frame ...")
    out_df = pd.DataFrame(
        {
    ...
    print(f"[score_val] wrote {len(out_df):,} rows -> {_OUTPUT_PARQUET_PATH}")
    ...
    sidecar = {
        ...
        "notes": (
            ...
            "score_val.py::_assert_persisted_column_reproduces_harness). Use "
            "p_calibrated for any analysis that treats the column as a probability "
            "(e.g. fitting an isotonic calibrator on val); p_raw is kept only for "
            "traceability back to the raw model output. This file mirrors "
            "score_test.py's output shape but for the VALIDATION split -- it is a "
            "re-scoring of the already-fitted artifact, not a new fit and not a "
            "second test touch (val was already used for model selection)."
        ),
    }
    with open(_OUTPUT_SIDECAR_PATH, "w", encoding="utf-8") as f:
        json.dump(sidecar, f, indent=2)
    print(f"[score_val] wrote sidecar metadata -> {_OUTPUT_SIDECAR_PATH}")
