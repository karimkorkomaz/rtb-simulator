...
**5. Baselines before complexity.** This report's "Results" section covers the logistic-regression
hashed-feature baseline, established and measured first. A LightGBM model, scored by the exact same
harness, is reported in "LR vs. LightGBM: isotonic-calibrated comparison" below, so its result has
...
## LR vs. LightGBM: isotonic-calibrated comparison

A LightGBM model (`backend/src/modeling/train_lgbm.py`, native-categorical encoding via
`features_lgbm.py`, same field set as the LR baseline, same temporal split, same downsampling rate
and seed, same shared evaluation harness) was trained after the LR baseline above and selected by
the identical protocol: an 8-configuration hand-picked hyperparameter grid
(`backend/data/metadata/ctr_lgbm_runs.jsonl`), selected on validation log loss, touched against
test exactly once (`backend/data/metadata/ctr_lgbm_test_evaluation_season2.json`, config
`lgbm_c3_deep`, `num_leaves=63`, `learning_rate=0.05`, `best_iteration=703`). This section fits an
isotonic calibrator on LightGBM's validation predictions -- mirroring the LR calibration above
exactly (validation only, applied to test exactly once, no retraining, no additional test-set
decision) -- and reports both models side by side.

**Reproduction of the already-recorded single test touch.** Before anything else, the persisted
LightGBM test predictions were regenerated by `backend/src/modeling/score_test_lgbm.py` (new; loads
`ctr_lgbm_season2.joblib`, never refits) and asserted, at runtime, to reproduce
`ctr_lgbm_test_evaluation_season2.json`'s recorded metrics to within `1e-9`: it did, exactly
(`auc=0.9292330416`, `log_loss=0.0046786926`, `ece=0.0004547699`). This is a reproduction of the
already-made test touch, not a second one. `backend/src/modeling/score_val_lgbm.py` (new,
mirroring `score_val.py`) closes the same gap on the val side -- no persisted per-row val
predictions existed for LightGBM before this work -- and was asserted, at runtime, to reproduce the
`val_selection_metrics` already recorded for the selected configuration to within `1e-9`: it did,
exactly (`auc=0.9308936778`, `log_loss=0.0044710230`, `ece=0.0004326421`).

**Fit/apply discipline (identical to the LR calibrator above).**
`backend/src/modeling/calibrate_isotonic_lgbm.py` (new) fits
`IsotonicRegression(out_of_bounds="clip", increasing=True)` via `.fit(p_val_calibrated, y_val)` on
`ctr_lgbm_season2_val.parquet` (1,745,722 rows, 1,392 positives) only, then transforms the
already-persisted `ctr_lgbm_season2_test.parquet` (1,657,338 rows, 1,369 positives) through the
fitted calibrator and scores it exactly once. Artifacts:
`backend/data/models/ctr_lgbm_season2_isotonic_calibrator.joblib` (fitted calibrator, same dict
shape as the LR one) and `backend/data/metadata/ctr_lgbm_isotonic_calibrator_season2.json` (full
before/after metrics, independent-qcut and matched-edge decile tables). The resulting `p_isotonic`
column was added to `ctr_lgbm_season2_test.parquet` **alongside** (not replacing) `y_true`, `p_raw`,
`p_calibrated`, `advertiser`, `timestamp` -- the pre-calibration columns are unchanged.

### Comparison table (test, single touch each)

| model | stage | AUC | log loss | ECE (10 quantile bins) | per-advertiser AUC (row-weighted mean) | per-advertiser AUC (simple mean) |
|---|---|---:|---:|---:|---:|---:|
| LR (`lr_hashed_C=0.1`) | before isotonic (`p_calibrated`) | 0.9228246833 | 0.0048598082 | 0.0003517860 | 0.9132 | 0.9131 |
| LR (`lr_hashed_C=0.1`) | **after isotonic (`p_isotonic`)** | **0.9226000297** | **0.0045612789** | **0.0000349450** | **0.9130** | **0.9130** |
| LightGBM (`lgbm_c3_deep`) | before isotonic (`p_calibrated`) | 0.9292330416 | 0.0046786926 | 0.0004547699 | 0.9173 | 0.9186 |
| LightGBM (`lgbm_c3_deep`) | **after isotonic (`p_isotonic`)** | **0.9287452705** | **0.0038874146** | **0.0000227927** | **0.9168** | **0.9179** |

Both models improve log loss and ECE markedly under isotonic recalibration (LR: ECE ~10x better,
log loss ~6% better; LightGBM: ECE ~20x better, log loss ~17% better) and both lose a small sliver
of AUC to isotonic's tie-collapse effect (documented for LR above) -- LightGBM loses more of it
(0.0004877711, vs. LR's 0.0002246536), because LightGBM's raw predicted probabilities in the lowest
deciles are far more tightly clustered (values on the order of `1e-7`-`1e-8`, see the decile ratio
table below), so isotonic's piecewise-constant step function collapses more of them onto shared
output values than it does for LR's more spread-out low-decile predictions.

**Per-advertiser AUC breakdown (compact secondary table, `p_calibrated`/`p_isotonic`, test):**

| advertiser | n | LR before | LR after | LightGBM before | LightGBM after |
|---|---:|---:|---:|---:|---:|
| 1458 | 447,493 | 0.9858 | 0.9855 | 0.9871 | 0.9869 |
| 3386 | 392,901 | 0.7412 | 0.7405 | 0.7497 | 0.7492 |
| 3427 | 390,398 | 0.9739 | 0.9736 | 0.9714 | 0.9708 |
| 3358 | 335,310 | 0.9458 | 0.9460 | 0.9539 | 0.9534 |
| 3476 | 91,236 | 0.9187 | 0.9194 | 0.9312 | 0.9293 |

LightGBM ranks better than LR within 4 of the 5 advertisers (1458, 3386, 3358, 3476, before
isotonic: +0.0013 to +0.0125 AUC) and worse within only 1 (3427, -0.0025 AUC), consistent with the
pooled and per-advertiser-mean pictures both favoring LightGBM. Isotonic recalibration moves every
advertiser's AUC by at most ~0.0018 in either direction for both models -- a magnitude/monotone-tie-
collapse effect, not a ranking-quality change, and not large enough to flip any advertiser's
before/after winner.

**The AUC difference between the two calibrated models is 0.0061, not 0.0066.** Comparing the two
models on the same basis -- both after isotonic, since that is the probability either model would
actually hand to a bidding function -- LightGBM's calibrated test AUC (0.9287452705) exceeds LR's
calibrated test AUC (0.9226000297) by **0.0061452408**, i.e. **0.0061** to 4 decimals. A naive
comparison of LightGBM's *pre*-isotonic AUC (0.9292330416, the recorded single-touch figure in
`ctr_lgbm_test_evaluation_season2.json`, reproduced above) against LR's *post*-isotonic
AUC (0.9226000297) gives 0.0066330119 (0.0066) -- but that mixes an uncalibrated number for one
model with a calibrated number for the other, which is not the comparison that matters once both
models are calibrated for actual use. The reason the two deltas differ (0.0061 vs. 0.0066) is
exactly the asymmetric tie-collapse noted above: isotonic costs LightGBM about twice as much AUC
(0.00049) as it costs LR (0.00022), so the calibrated-vs-calibrated gap is correspondingly smaller
than the gap computed from LightGBM's uncalibrated number. **0.0061 is the number that should be
quoted as "LightGBM vs. LR, both calibrated."**

### Relative calibration: LightGBM predicted/observed ratio by decile (test)

Same construction as the LR "Relative calibration" table above
(`modeling.evaluate.expected_calibration_error`'s quantile bins), before isotonic:

| decile | n | mean predicted | mean actual | ratio (pred/obs) |
|---:|---:|---:|---:|---:|
| 1 | 165,734 | 6.307323e-08 | 1.206753e-05 | 0.0052 |
| 2 | 165,734 | 2.033071e-07 | 1.810129e-05 | 0.0112 |
| 3 | 165,734 | 4.463926e-07 | 3.620259e-05 | 0.0123 |
| 4 | 165,736 | 8.383439e-07 | 7.240431e-05 | 0.0116 |
| 5 | 165,731 | 1.475409e-06 | 1.327452e-04 | 0.0111 |
| 6 | 165,734 | 2.568786e-06 | 1.629117e-04 | 0.0158 |
| 7 | 165,733 | 4.619615e-06 | 2.654873e-04 | 0.0174 |
| 8 | 165,734 | 9.108031e-06 | 3.499584e-04 | 0.0260 |
| 9 | 165,734 | 2.292673e-05 | 6.395791e-04 | 0.0358 |
| 10 | 165,734 | 3.670281e-03 | 6.570770e-03 | 0.5586 |

**This is the finding flagged going in: LightGBM's pooled ECE (0.0004547699) hides a
multiplicative miscalibration far worse than LR's.** LR's worst decile ratio (test, before
isotonic) was 3.95x over-prediction (bin 4, "Relative calibration" section above). LightGBM's
lowest 9 deciles all systematically **under**-predict, by two orders of magnitude at the extreme:
decile 1's mean predicted probability (`6.3e-08`) is **~192x smaller** than the observed click rate
in that bin (`1.2e-05`, ratio 0.0052) -- i.e. LightGBM assigns almost no probability mass at all to
the bottom 10% of its own predicted-probability range, even though that range still contains real
clicks (2 of them, out of 165,734 rows). Deciles 2-9 are less extreme but still under-predict by
28x-90x (ratios 0.0112-0.0358, i.e. `1/ratio`), tapering monotonically toward decile 9 (28x) as the
predicted probability rises. Only decile 10 (the top 10% of predictions, carrying 1,089 of 1,369
test positives) is in the same order of magnitude as observed, and even there it under-predicts by
~1.8x (ratio 0.5586). Mean absolute deviation of the
ratio from 1.0, averaged across all 10 deciles, is **~0.93** before isotonic (vs. LR's ~1.00,
computed the same way over LR's before-isotonic ratios above) -- both models are, in this
multiplicative sense, severely miscalibrated before any post-hoc correction, and pooled ECE alone
would not have surfaced either fact.

**Why pooled ECE stays tiny despite this.** ECE is a row-count-weighted average of *absolute* gaps
(`|mean_predicted - mean_actual|`), and every quantity in deciles 1-9 is on the order of `1e-4` to
`1e-8` -- multiplying a probability by 0.01 or by 100 still changes the absolute gap by only a few
`1e-6` at this base rate, so ECE (and, to a lesser extent, log loss) is nearly blind to this
particular failure mode. A bidding function computing `bid = value * p_click` on the raw,
uncalibrated LightGBM output would underbid by roughly two orders of magnitude for any impression
whose predicted probability falls in the bottom decile -- a much larger practical distortion than
anything found in the LR baseline's relative-calibration analysis.

### After isotonic: matched-edge decile table (test)

Same matched-edge construction as the LR section above -- bin membership is derived once, from
`p_calibrated` (before), and reused unchanged to aggregate `p_isotonic` (after), so `n` and
`observed_click_rate` are identical across the before/after columns for a given bin and only
`mean_predicted`/the ratio can differ:

| bin | edge range (`p_calibrated`, before) | n | mean pred before | mean pred after | observed click rate | n_pos | pred/obs before | pred/obs after |
|---:|---|---:|---:|---:|---:|---:|---:|---:|
| 1 | 8.04e-10 -- 1.20e-07 | 165,734 | 6.307323e-08 | 2.958632e-06 | 1.206753e-05 | 2 | 0.0052 | 0.2452 |
| 2 | 1.20e-07 -- 3.04e-07 | 165,734 | 2.033071e-07 | 2.094766e-05 | 1.810129e-05 | 3 | 0.0112 | 1.1572 |
| 3 | 3.04e-07 -- 6.12e-07 | 165,734 | 4.463926e-07 | 3.802350e-05 | 3.620259e-05 | 6 | 0.0123 | 1.0503 |
| 4 | 6.12e-07 -- 1.10e-06 | 165,736 | 8.383439e-07 | 6.289671e-05 | 7.240431e-05 | 12 | 0.0116 | 0.8687 |
| 5 | 1.10e-06 -- 1.92e-06 | 165,731 | 1.475409e-06 | 9.680927e-05 | 1.327452e-04 | 22 | 0.0111 | 0.7293 |
| 6 | 1.92e-06 -- 3.36e-06 | 165,734 | 2.568786e-06 | 1.656592e-04 | 1.629117e-04 | 27 | 0.0158 | 1.0169 |
| 7 | 3.36e-06 -- 6.23e-06 | 165,733 | 4.619615e-06 | 2.018169e-04 | 2.654873e-04 | 44 | 0.0174 | 0.7602 |
| 8 | 6.23e-06 -- 1.31e-05 | 165,734 | 9.108031e-06 | 3.319292e-04 | 3.499584e-04 | 58 | 0.0260 | 0.9485 |
| 9 | 1.31e-05 -- 4.00e-05 | 165,734 | 2.292673e-05 | 6.685866e-04 | 6.395791e-04 | 106 | 0.0358 | 1.0454 |
| 10 | 4.00e-05 -- 9.89e-01 | 165,734 | 3.670281e-03 | 6.497162e-03 | 6.570770e-03 | 1,089 | 0.5586 | 0.9888 |

Isotonic recalibration is a dramatic fix for LightGBM's under-prediction problem: decile 1's ratio
moves from 0.0052 to 0.2452 (still under-predicting, but by ~4x rather than ~192x), and deciles
3, 6, 8, 9, 10 all land within ~5% of perfect calibration (ratios 0.95-1.05) after the transform;
bin 2 lands within 16% (ratio 1.16). Bins 4, 5, and 7 remain the furthest off after calibration
(ratios 0.73-0.87, still 13-27% away from 1.0), all three under-predicting.
Mean absolute deviation of the ratio from 1.0 falls from **~0.93 to ~0.17** across the 10 bins --
roughly a **5.4x tightening**, comparable in kind to the ~4.6x tightening the LR calibrator achieved
above, even though LightGBM started from a much worse (under-, not over-predicting) baseline. Bin 2
is the exception in the same direction the LR table flagged for its own low-count bins: it moves
from 0.0112 to 1.1572 -- overshooting 1.0 rather than landing near it, on only 3 observed positives,
the noisiest bin in the table.

### Scope and provenance

- **Fit split: validation only, applied once**, for both models, using the identically-named
  discipline documented in "Post-hoc isotonic recalibration" above.
- **New scripts (this section only; the LR calibration pipeline above is unmodified):**
  `backend/src/modeling/score_val_lgbm.py`, `backend/src/modeling/calibrate_isotonic_lgbm.py`. Both
  mirror their LR counterparts (`score_val.py`, `calibrate_isotonic.py`) function-for-function.
- **New artifacts:** `backend/data/predictions/ctr_lgbm_season2_val.parquet` (+ `.json` sidecar),
  `backend/data/predictions/ctr_lgbm_season2_test.parquet` (+ `.json` sidecar, regenerated by
  `score_test_lgbm.py` and then extended in place with a `p_isotonic` column by
  `calibrate_isotonic_lgbm.py` -- `y_true`/`p_raw`/`p_calibrated`/`advertiser`/`timestamp` are
  unchanged), `backend/data/models/ctr_lgbm_season2_isotonic_calibrator.joblib`,
  `backend/data/metadata/ctr_lgbm_isotonic_calibrator_season2.json`.
- **No LR artifact was refit, overwritten, or re-touched against test to produce this section.**
  The LR per-advertiser-after-isotonic figures in the comparison table above (0.9130 row-weighted,
  0.9130 simple) were not previously computed/recorded anywhere in this doc; they were derived here
  by loading the already-persisted `ctr_lr_baseline_season2_isotonic_calibrator.joblib` and
  `.predict()`-ing it against the already-persisted `ctr_lr_baseline_season2_test.parquet`'s
  `p_calibrated` column -- a pure transform of already-persisted, already-fit artifacts, not a new
  fit and not a new test touch.
...
- `backend/src/modeling/features_lgbm.py`, `train_lgbm.py`, `score_test_lgbm.py` -- LightGBM
  native-categorical encoding, training/selection/test-touch orchestration, and the test-side
  reproduction-of-the-single-touch script (see "LR vs. LightGBM: isotonic-calibrated comparison").
- `backend/src/modeling/score_val_lgbm.py`, `calibrate_isotonic_lgbm.py` -- new; mirror
  `score_val.py`/`calibrate_isotonic.py` for the LightGBM artifact (validation-only isotonic fit,
  applied once to test).
- `backend/data/metadata/ctr_lgbm_runs.jsonl`, `ctr_lgbm_test_evaluation_season2.json`,
  `ctr_lgbm_isotonic_calibrator_season2.json` -- generated, traceable LightGBM run records.
- `backend/data/models/ctr_lgbm_season2.joblib`, `ctr_lgbm_season2_isotonic_calibrator.joblib` --
  fitted booster + category-encoder state, and the fitted isotonic calibrator.
- `backend/data/predictions/ctr_lgbm_season2_val.parquet`, `ctr_lgbm_season2_test.parquet` (+ JSON
  sidecars) -- persisted per-row LightGBM predictions (`y_true`, `p_raw`, `p_calibrated`, and, on
  the test parquet only, `p_isotonic`), `advertiser`, `timestamp`.
- `reports/figures/part4_lgbm_calibration_val.{png,svg}`, `part4_lgbm_calibration_test.{png,svg}`.

## Explicitly out of scope here

- LightGBM was out of scope for the LR-baseline sections above (the "Results" through "Post-hoc
  isotonic recalibration" sections, and the leakage/ablation/placement-overlap analyses, are all
  LR-only); it is now trained and isotonic-calibrated in "LR vs. LightGBM: isotonic-calibrated
  comparison" below.
